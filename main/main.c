...
void TouchPosition(TFT_t *dev, FontxFile *fx, int width, int height, TickType_t timeout)
{
	...
	while (1)
	{
		if (touch_getxy(dev, &_xp, &_yp) == false)
			break;
	} // end while
...
esp_err_t ConvertCoordinate(TFT_t *dev, int xp, int yp, int *xpos, int *ypos)
{
	...
	{
		if (xp < dev->_min_xp && xp > dev->_max_xp)
			return ESP_FAIL;
	}
	else
	{
		if (xp < dev->_max_xp && xp > dev->_min_xp)
			return ESP_FAIL;
	}
	if (dev->_max_yp > dev->_min_yp)
	{
		if (yp < dev->_min_yp && yp > dev->_max_yp)
			return ESP_FAIL;
	}
	else
	{
		if (yp < dev->_max_yp && yp > dev->_min_yp)
			return ESP_FAIL;
	}
...
void app_main(void)
{
	...
	xTaskCreate(&print_memory, "memory", 1024 * 6, NULL, 2, NULL);
